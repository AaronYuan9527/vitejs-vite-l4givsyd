...
const MarketAnalysisSection = ({ transactions }) => {
    ...
    const { monthlyData, summary } = useMemo(() => {
        ...
        transactions.forEach(t => {
            ...
            let isKey1 = false;
            if (viewMode === 'region') {
                const region = normalizeCountry(t.country, t.detectedCurrency);
                isKey1 = region === '台灣';
            } else {
                isKey1 = t.status === '新客戶';
            }
        ...
        <div className="overflow-y-auto p-0 flex-1 bg-white">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {sortedTransactions.length === 0 ? (<tr><td colSpan={DATA_FIELDS.length} className="px-6 py-16 text-center text-slate-400">此區間無資料</td></tr>) : (
                sortedTransactions.map((t, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                    <td className="px-6 py-4 text-slate-500 font-mono text-xs">{formatDate(t.date)}</td>
                    <td className="px-6 py-4 font-bold text-slate-700 font-mono">{formatCurrency(t.amount)}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col animate-in zoom-in-95 duration-300 border border-white/20 overflow-hidden">
                ...
                <div className="overflow-y-auto p-0 flex-1 bg-white">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50/80 text-slate-500 font-semibold sticky top-0 shadow-sm z-10 backdrop-blur-md">
                            <tr><th className="px-6 py-4 w-16 text-center">排名</th><th className="px-6 py-4">{isAgent ? '業務' : '產業'}</th><th className="px-6 py-4 text-right">業績</th><th className="px-6 py-4 text-right">件數</th></tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                            {data.map((item, idx) => (
                                <tr key={item.name} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 text-right font-bold text-slate-700 font-mono">{formatCurrency(item.revenue)}</td>
                                    <td className="px-6 py-4 text-right text-slate-500">{item.count}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
const Dashboard = ({ transactions, loading, error, onRefresh, user }) => {
  ...
  const { availableYears, availableAgents, availableStatuses, availableIndustries } = useMemo(() => {
    ...
    transactions.forEach(t => {
        if (t.date && typeof t.date === 'string') years.add(t.date.substring(0, 4));
  ...
  const filteredData = useMemo(() => {
    return transactions.map(t => {
        ...
        const isLocal = normalizedCurrency.includes('TWD') || normalizedCurrency.includes('NT') || normalizedCurrency.includes('臺幣');
        
        if (!isLocal) { finalAmount = finalAmount * exchangeRate; isUSD = true; }
        ...
        if (clientName && clientName.toLowerCase().includes('iherb')) {
            displayAgentName = 'Iherb (獨立客戶)';
        }
        ...
        return { 
            ...
            detectedCurrency: t.currency || '臺幣', 
            isUSD,
            status: calculatedStatus 
        };
    }).filter(t => t !== null).filter(t => {
